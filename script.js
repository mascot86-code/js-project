...
Vue.component('goods-item', {
    props: ['good'],
    template: '<div class="goods-item"><h3>{{good.title}}</h3><p>{{good.price}}</p><button :id="good.id" v-on:click="addBasket(event)">Добавить</button></div>'
})
...
Vue.component('basket-item', {
    props: ['good'],
    template: '<div class="basket-item"><button :id="good.id" v-on:click="deleteItem(event)">&times;</button><div class="basket-item-info"><h3>{{good.title}}</h3><p>{{good.price}}</p></div></div>'
})

Vue.component('search', {
    props: [],
    template: '<div class="search"><input type="search" v-on:keydown.enter="filterGoods" v-model="app.searchLine" placeholder="Что ищем?"></div>'
});
